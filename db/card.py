...
def post_card(new_card_name, new_card_board_name, new_card_description, new_card_estimation):
    ...
    params_board_id = (new_card_board_name,)
    try:
        with get_cursor() as cursor:
    ...
    query_post_card = """
        ...
        RETURNING *;"""
    params_post_card = (1, board_id, dt_now, new_card_description, new_card_estimation, 1, new_card_name)
    try:
        with get_cursor() as cursor:
...
def get_estimation_card(board_name_in_query, column_name_in_query, assignee_in_query):
    ...
    """
    params_estimation = (assignee_in_query, column_name_in_query)
    try:
        with get_cursor() as cursor:
    ...
    query_card = """
        ...
        AND user_name = %s;
    """
    params_card = (board_name_in_query, column_name_in_query, assignee_in_query)
    try:
        with get_cursor() as cursor:
